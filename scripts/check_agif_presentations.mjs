...
assert.equal((day2Deck.match(/<section class="slide\b/g) || []).length, 31, 'Day 2 must expose 31 retained slides');
assert.equal((day2Deck.match(/aria-label="Slide \d+ of 31:/g) || []).length, 31, 'every Day 2 slide needs navigation metadata');
assert.doesNotMatch(
...
assert.match(day2Deck, /id="slideCounter">1 \/ 31/, 'Day 2 counter must use the real slide total');
...
for (const slideNumber of [7, 18, 28]) {
  const slide = slideMarkup(day2Deck, slideNumber, 31);
...
const day2Slide7 = slideMarkup(day2Deck, 7, 31);
