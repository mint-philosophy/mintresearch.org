...
desktop scale for any slide whose content exceeds its viewport, then verifies
the rendered result against the fixed navigation edge. Prepared text/font pairs
are cached across resize runs, hidden tooltip copy is excluded, and the pinned
Pretext dependency has a DOM-measurement fallback. Slides that fit stay at full
size; the fitter preserves at least `24px` substantive text and `14px` labels,
and mobile keeps native per-slide scrolling. At `2560x1080` in the branded shell,
all ten slides must have `scrollHeight <= clientHeight`, the slide plane must end
exactly at the top of the deck navigation, and no slide should be reduced unless
it otherwise overflows.
