...
`public/FDC-deck.html` uses `@chenglou/pretext` to predict the largest readable
desktop scale for each slide, then verifies the rendered result against the
fixed navigation edge. The upper scale is derived from the deck's actual iframe
slide plane, so the branded shell's banner, sidebar, status line, toolbar,
padding, and borders reduce the type scale even when the outer display remains
large. An unframed `2560x1080` presentation is the authored `1.0` reference; at
`2560x1080` in the branded shell, the `2294x794` slide plane caps ordinary slides
at `0.81` before slide-specific readability floors are applied. Prepared
text/font pairs are cached across resize runs, hidden tooltip copy is excluded,
and the pinned Pretext dependency has a DOM-measurement fallback. The fitter
preserves at least `24px` substantive text as the slide-wide floor. Structural
labels retain their own `14px` rendered floor without forcing the title and body
back toward full size. The versioned iframe and fit-math URLs prevent a stale
subframe from surviving a deployment. Mobile keeps native per-slide scrolling,
and the shell measures both banner and status-bar height instead of estimating
the available frame. Run `npm run check:fdc-fit` for the deterministic
full-screen, framed desktop, and `390x844` mobile checks.
