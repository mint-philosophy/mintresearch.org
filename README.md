...
preserves at least `24px` substantive text and `14px` labels, while mobile keeps
native per-slide scrolling. The shell measures both banner and status-bar height
instead of estimating the available frame. Run `npm run check:fdc-fit` for the
deterministic full-screen, framed desktop, and `390x844` mobile checks.
